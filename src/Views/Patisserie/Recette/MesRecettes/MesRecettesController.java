            ...
            for(Recette rec : listRec)
            {
                ...
                img.setOnMouseClicked(e->{
                    try {
                        ...
                        FXMLLoader loaderSingleRecette = new FXMLLoader(getClass().getResource("../../../Patisserie/Recette/ModifierRecette/ModifierRecette.fxml"));
                        Node SingleRecette = loaderSingleRecette.load();
                        ModifierRecetteController CsR = loaderSingleRecette.getController();
                        
                        CsR.setCsr(CsR);
                        
                        FXMLLoader loaderinfoRecette = new FXMLLoader(getClass().getResource("../../../Patisserie/Recette/ModifierRecette/infoRecette.fxml"));
            ...
            img.setOnMouseClicked(e->{
                try {
                    ThreadService ts = new ThreadService();
                        if (ts.ThreadExiste(rec.getIdRec().toString()) == null )
                            ts.AjouterThread(new Entity.Thread(rec.getIdRec().toString()));
                    FXMLLoader loaderSingleRecette = new FXMLLoader(getClass().getResource("../../Patisserie/Recette/SingleRecette/SingleRecette.fxml"));
                    Node SingleRecette = loaderSingleRecette.load();
                    body.getChildren().clear();
                    body.getChildren().add(SingleRecette);
                } catch (IOException ex) {
